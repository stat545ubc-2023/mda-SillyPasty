...
The `steam_games` contains 21 columns and 40833 rows, the column name and type 
is shown below. There are 4 columns with `dbl` type and the rest are all `chr` type.
```{r}
...
The `vancouver_trees` contains 20 columns and 146,611 rows, the column name and type 
is shown below. There are 7 columns with `dbl` type, 1 column with date type and
the rest are all `chr` type.
```{r}

dim(vancouver_trees)
glimpse(vancouver_trees)
```
The `parking_meters` contains 22 columns and 10,032 rows, the column name and type 
is shown below. There are 3 columns with `dbl` type, 1 column with date type and
the rest are all `chr` type.
```{r}
...
```
The `cancer_sample` contains 32 columns and 569 rows, the column name and type 
is shown below. There are 1 column with `chr` type and the rest are all `dbl` type.
```{r}
...
<!-------------------------- Start your work below ---------------------------->
I would like to choose `vancouver_trees` one for several reasons:

1. The dataset is complex enough, with enough columns and rows for some data research

2. The dataset contains different type such as `chr`, `dbl` and `date`

3. Personally, I really wonder the distribution of trees in vancouver as the tree
  in campus is really beautiful.
<!----------------------------------------------------------------------------->
...
<!-------------------------- Start your work below ---------------------------->
How the number of trees planted each year changes over time?
<!----------------------------------------------------------------------------->
...
*3.  Investigate how many missing values there are per variable. Can you find a way to plot this?*
```{r}
missing_values <- sapply(vancouver_trees, function(x) sum(is.na(x)))
barplot(missing_values, 
        names.arg = names(missing_values), 
...
*4.  Explore the relationship between 2 variables in a plot.*
The relation between street and the unique tree there.

```{r}
genus_count_by_street <- vancouver_trees %>%
  group_by(on_street, genus_name) %>%
  summarise(count = n())

head(genus_count_by_street)
barplot(genus_count_by_street$count, 
        beside = TRUE, 
        col = length(unique(vancouver_trees$genus_name)),
        main = "Genus Count by Street Block",
        xlab = "Street Blocks",
        ylab = "Count")
...
*6.  Use a boxplot to look at the frequency of different observations within a single variable. You can do this for more than one variable if you wish!
*
The distribution of diameter among different species. As the species' amount is large, only choose 10 species.

```{r}

top_10_species <- vancouver_trees %>%
...
data_filtered <- vancouver_trees %>%
  filter(species_name %in% top_10_species$species_name)


ggplot(data_filtered, aes(x = species_name, y = diameter)) +
  geom_boxplot(fill = "blue", color = "black") +
  labs(
    title = "Diameter vs. Top 10 Species Box Plot",
    x = "Species",
    y = "Diameter"
  ) +
  theme_minimal() +
...
ggplot(vancouver_trees, aes(x = species_name, y = diameter)) +
  geom_boxplot(fill = "blue", color = "black") +
  labs(
    title = "Diameter vs. Species Box Plot",
    x = "Species",
    y = "Diameter",
  ) +
  theme_minimal()
...
```{r}
my_tribble <- vancouver_trees %>%
  select(species_name, root_barrier, on_street, height_range_id, diameter, date_planted, longitude, latitude) %>%
  as_tibble()
head(my_tribble)
```
<!----------------------------------------------------------------------------->
...
For question 3, because I need to do some data analysis based on the value, I need to know if the column value is enough to support the analysis, such as print a plot.

For question 4, I plot the relation between street and the unique tree there.

For question 6, I analysis the distribution of diameter among different species with box plot. As box plot is useful to get the distribution information of data.

For question 7, because I do not need some columns such as `cultivar_name` so I just keep the data I want for future useage.
...
<!--- *****START HERE***** --->
1. Will the usage of root barrier affect the diameter and height of the tree?

2. What is the average diameter and height of each species? (Because the species of the trees are too many, only count top 10 species)

3. What is the relationship between tree age and height?

4. What is the height and diameter distribution according to the longitude?
<!----------------------------->
